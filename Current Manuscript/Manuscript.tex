...
This study addresses these gaps by empirically analyzing the instability of classification models used in recidivism risk prediction. Specifically, it examines how model predictions vary under perturbations in the training data and stochastic training conditions, evaluates instability alongside preddictive performance and reliability metrics, and identifies key factors that contribute to variability in predictions.

\section{Objectives of the Study} 
\subsection{General Objective} Evaluate the instability, performance, and reliability of classification models used in recidivism risk prediction.
...
\begin{enumerate}
 ...
 \item Evaluate each model for each type of instability using metrics for instability, performance, reliability, and feature contributions using the prediction values obtained. 
...
\tab This study defines and focuses on two forms of instability, namely dataset instability and stochastic instability. Instability is evaluated empirically through model retraining under controlled perturbations, such as bootstrapped sampling of the training dataset and variation in stochastic initialization through changes in its random seed. The analysis measures predictive performance, reliability, and instability across multiple dimensions such as predicted probabilities, ranking of individuals, classification outcomes, and overall predictive performance and reliability.\\
...
To evaluate instability in each model, multiple metrics will be used to measure variability across different aspects of model behavior such as prediction score distribution, scores, labels, and ranking, as well as performance, reliability, and feature contributions. These metrics will be computed through multiple training runs and measured for both dataset instability and stochastic instability.\\
...
\end{itemize}
\tab These variables will be used consistently across the instability and performance metrics defined in this study.

\begin{itemize}
  \item \textbf{Score Distribution-Level Instability: 95\% Stability Interval (SI)}\\
    \tab This will visualize the range within which predicted probabilities for each defendants are expected to fall across repeated runs~\cite{riley2023stability}. Wider intervals will indicate greater instability in probability scores. The range of the interval is described through the following formula: \\
    \[
    \text{SI}_{95\%,d} =
    \left[
  ...
  \item \textbf{Score-Level Instability: Mean Absolute Prediction Error (MAPE)}\\
    \tab This metric is used to quantify the average absolute difference in predicted probabilities for each defendant across runs~\cite{riley2023stability}. Higher MAPE values for a model will indicate greater variability in predicted risk scores, suggesting higher instability.\\
...
\begin{itemize}
  \item \textbf{Brier Score (BS)}\\
    \tab This will assess the accuracy of predicted probabilities~\cite{rufibach2010use}. The standard deviation of Brier scores across runs will also be computed to evaluate instability in probability accuracy. Higher variability will suggest inconsistency in the quality of probabilistic predictions, including potential variation in calibration.\\
  ...
  \tab This visualization assesses how well predicted probabilities align with observed outcome frequencies. A curve close to the diagonal indicates good calibration, while deviations suggest overconfidence or underconfidence in predictions.
...
However, aggregated performance metrics alone do not reflect the reliability of  defendant predictions, which is critical in risk-sensitive applications. Thus, this study will also incorporate a reliability metric to assess the consistency and trustworthiness of model outputs. 
...
Obj. 3
& Evaluate each model for each type of instability using metrics for instability, performance, reliability, and feature contributions using the prediction values obtained.
&
...
(4) Use SHAP analysis to identify which features exhibit variability in their contributions across runs. \newline
(5) Save all computed instability, performance, reliability, and feature contribution results and graphs for analysis.
