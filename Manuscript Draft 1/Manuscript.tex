...
\tab% add tab
Instability in machine learning (ML) refers to how varied a model's predictions are, either when retrained under small perturbations such as training data or due to nondeterministic factors in the learning process~\cite{sun2015stability}. A model that has greater prediction variability indicates instability, which can undermine its reliability. This is particularly critical in risk-sensitive situations as inconsistent predictions can lead to catastrophic outcomes such as harm to humans or major economic loss~\cite{zhang2024reliability}. Prior studies have shown that both small changes in training data and stochastic factors such as random seed initialization can lead to variations in predictions~\cite{riley2023stability, lopez2022instability}. For clarity, this paper distinguishes two types of instability: dataset instability, which arises from small perturbations in the training data, and stochastic instability, which is caused by nondeterministic factors that affect the model during training. \\
...
These models are not only theoretical but applied in official sentencing decisions. A prime example is the COMPAS algorithm, which has influenced the sentencing of some court cases such as the Loomis v State case in 2013~\cite{loomis2016} and the Samsa v State case in 2014~\cite{samsa2014}. Due to the rise in the use of ML models in real-life criminal justice, concerns had begun to arise regarding its fairness. In 2016, Propublica published a report detailing how they analyzed COMPAS for its fairness with regards to race and found that it was more likely to misclassify Black defendants as higher risk than White defendants, while misclassifying White defendants as low risk than Black defendants~\cite{propublica2016analysis}. Critiques of the report rebutted that when measured not on the separation of race but on the defendant's likelihood to recidivate, COMPAS performed fairly~\cite{dieterich2016compas, corbett2017algorithmic, flores2016false}. Due to the scandal, subsequent studies arose focusing on measuring the performance~\cite{tollenaar2013method, travaini2022machine} and fairness~\cite{dressel2018accuracy, tolan2019machine, wadsworth2018achieving} of ML models used to predict recidivism such as COMPAS and MnSTARR, as well as other ML methods such as random forest and SVMs.\\



any instability that results to different risk assessments for the same defendant could lead to unfair legal outcomes.  and thus 
...
% classification model that predicts a discrete or categorical outcome (in this case, pwede hiya binary classification)
% linear that examines the relationships between 
